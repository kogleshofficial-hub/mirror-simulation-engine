...
export function validateSimulationModel(value: unknown): SimulationModel | null {
  ...
  const events: SimulationEvent[] = Array.isArray(raw.events)
    ? raw.events
        .map(event => {
          const e = event as Record<string, unknown>
          const type: SimulationEvent['type'] =
            e.type === 'add_resource'
              ? 'add_resource'
              : e.type === 'demand_change'
                ? 'demand_change'
                : 'remove_resource'
          const target: SimulationEvent['target'] =
            e.target === 'subject' ? 'subject' : 'resource'
        ...
        .filter(e => e.minute <= durationMinutes)
        .slice(0, 30)
    : []
  ...
  return {
    ...
    subject: {
      label: String(subject.label).slice(0, 80),
      count: Math.max(0, subjectCount),
    },
