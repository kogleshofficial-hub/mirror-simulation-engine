...
function parse(text:string):Model{
 const n=(re:RegExp, fallback:number)=>{const m=text.match(re);return m?Math.max(0,Number(m[1])):fallback}
 ...
 const duration=n(/(\d[\d,]*)\s*(?:hours|hr)/i,2)
 const loss=/leaves?|leaving|unavailable|gone|loses?|removed/i.test(text)?Math.round(duration*30):null
...
function result(model:Model, minute:number){
 ...
 const arrivals=Math.min(model.attendees,Math.ceil(model.attendees/model.duration)*minute)
 const served=Math.min(arrivals,capacity*minute)
 const queue=Math.max(0,arrivals-served)
 const completion=Math.ceil(model.attendees/Math.max(1,capacity))
 return {workers,capacity,queue,served,completion,bottleneck:workers<=model.desks?'Organizers':'Registration desks'}
}

export default function Home(){
 const [text,setText]=useState(demo)
 const [model,setModel]=useState<Model>(()=>parse(demo))
 const [minute,setMinute]=useState(0)
 const [running,setRunning]=useState(false)
 const [error,setError]=useState('')
 const state=useMemo(()=>result(model,minute),[model,minute])
 const run=()=>{const m=parse(text);if(m.attendees<=0){setError('I can’t run this scenario yet. Add at least one attendee.');return}if(m.organizers<=0||m.desks<=0){setError('I can’t run this scenario yet. Define at least one organizer and one registration desk.');return}setError('');setModel(m);setMinute(0);setRunning(true);let t=0;const id=window.setInterval(()=>{t+=1;setMinute(t);if(t>=m.duration*60){window.clearInterval(id);setRunning(false)}},80)}
 ...
 return <main className="shell">
  ...
  <section className="workspace">
   <div className="panel input-panel"><div className="label">01 / Scenario</div><textarea className="textarea" value={text} onChange={e=>setText(e.target.value)} aria-label="Scenario description"/><div className="actions"><button className="primary" onClick={run}>{running?'Running…':'Run simulation'}</button><button className="secondary" onClick={reset}>Reset</button></div>{error&&<div className="error">{error}</div>}<div className="demo">TRY THE DEMO<button onClick={()=>setText(demo)}>School event · 200 attendees · 3 organizers · 2 desks</button></div></div>
   <div className="panel sim"><div className="simhead"><div><div className="label">02 / Live model</div><div className="simtitle">Registration flow</div><div className="sub">State updates every simulated minute</div></div><div className="clock">{String(Math.floor(minute/60)).padStart(2,'0')}:{String(minute%60).padStart(2,'0')} / {String(model.duration).padStart(2,'0')}:00</div></div>
    ...
    <div className="explain"><strong>Why this result?</strong> MIRROR is calculating capacity from active organizers and registration desks, applying arrivals over simulated time, then tracking the queue after each state transition. Same model, same rules, same result.</div>
   </div>
  </section>
  <section className="model"><div className="label">03 / What MIRROR models</div><div className="modelgrid"><div className="modelcard"><h3>Entities & resources</h3><p>Attendees, organizers, desks and other modeled participants become explicit state variables.</p></div><div className="modelcard"><h3>Rules & transitions</h3><p>Capacity, arrivals and resource changes are executed by deterministic state transitions rather than generated answers.</p></div><div className="modelcard"><h3>Explainable outcomes</h3><p>Queues, throughput, bottlenecks and timing are visible so you can inspect why the result happened.</p></div></div></section>
