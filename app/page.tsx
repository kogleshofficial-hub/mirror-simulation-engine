...
function Chart({ states, minute }: { states: SimulationState[]; minute: number }) {
  const width = 760, height = 190, pad = 22, max = Math.max(1, ...states.map(s => s.queue))
  ...
  return <div className="chart-wrap"><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="System pressure over simulated time"><line x1={pad} x2={width-pad} y1={height-pad} y2={height-pad} className="axis"/><polyline points={points} className="chart-line"/><line x1={x} x2={x} y1={pad} y2={height-pad} className="cursor"/></svg><div className="chart-axis"><span>00:00</span><span>{clock(Math.round(states.length / 2))}</span><span>{clock(states.at(-1)?.minute ?? 0)}</span></div></div>
}
